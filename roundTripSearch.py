...
class Graph:
    ...
    def read_themes_from_file(self, file_name):
    ...
    def assign_custom_theme_list(self,themes):
...
def RoundTripRoadTrip(startLoc, 
    ...
    graph.assign_custom_theme_list(themes_list)
...
def call_llm(narrative):
    """Call the OpenAI language model to generate a narrative based on the road trip information.

    We call ChatGPT-3.5-turbo model to generate a narrative based on the road trip information.
    The api key is stored as an environment variable in the form of OPENAI_API_KEY.
    
    In your .bashrc or .bash_profile file or .zshrc file, add the following line:
    ...
    Args:
        narrative (srt): narrative prompt for the road trip.

    Returns:
        str: The generated narrative for the road trip.
    """
...
def Give_Narrative(road_trip: RoadTrip):
    ...
    narrative = f"\nThe round trip road trip starts from {road_trip.startLoc} and embarks on a memorable journey across various cities and landscapes."
    narrative += f"\nThe road trip consists of {len(road_trip.edges)} legs. "
    narrative += '''\nGenerate a narrative of the roadtrip in an engaging, enticing, and informative manner, from the information provided. 
    \nFor each journey, suggest popular attractions and activities based on your knowledge if any. 
    \nIf any, narrate any notable landscapes, cultural elements, or unique experiences along the way. '''
    
    narrative += "\n\nThe road trip itinerary is as follows:"
    for i, edge in enumerate(road_trip.edges, 1):
        location1 = edge.location1.label
        location2 = edge.location2.label
...
if __name__ == '__main__':
    ...
    RoundTripRoadTrip('NashvilleTN', 'Locations.csv', 'Edges.csv','Attractions.csv',themes_list, HandCraftedTree1,20, 80, "results.txt", required_locations, forbidden_locations)
    # T = HandCraftedTree1()
